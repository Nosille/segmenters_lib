...
void GroundRANSACSegmenter::segment(
    ...
    *cloud = cloud_in;
    ...
    sac_estimator_.setInputCloud(cloud);
    sac_estimator_.segment(*temp_indices, *coefficients);

    if (temp_indices->indices.size() > 0) {
      ground_indices->indices.insert(ground_indices->indices.end(), temp_indices->indices.begin(), temp_indices->indices.end()); 
    }
    ...
    for (size_t seg = 0u; seg < segment_number; ++seg) {
        ...
        sac_segmenter_.segment(*temp_indices, *coefficients);

        if (temp_indices->indices.size() > 0) {
          ground_indices->indices += temp_indices->indices; 
        }
    }
#endif
    ...
    for (size_t pt = 0u; pt < points.size(); ++pt) {
        ...
        if (pt == segment_size * segment_id || pt == (points.size() - 1)) {
            seg.setInputCloud(cloud);
            seg.segment(*temp_indices, *coefficients);
    ...
    if (temp_indices->indices.size() > 0) {
        ...
        seg.setInputCloud(cloud_nonground);
        seg.segment(*temp_indices, *coefficients);
        if (temp_indices->indices.size() > ground_removal_2nd_min_size_) {
            ground_indices->indices += temp_indices->indices; 
        }
